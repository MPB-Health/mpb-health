...
export default function SOPLibrary() {
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold text-th-text-primary">SOPs & Playbooks</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
          ...
          {filteredDocuments.length > 0 ? (
            filteredDocuments.map((doc) => (
              <button
                ...
                className="w-full bg-surface-primary rounded-xl border border-th-border p-5 text-left hover:border-th-accent-300 hover:shadow-md transition-all"
              >
                <div className="flex items-start justify-between">
                  <div className="flex items-start space-x-4">
                    <div className="w-12 h-12 bg-th-accent-100 rounded-lg flex items-center justify-center flex-shrink-0">
                      <FileText className="w-6 h-6 text-th-accent-600" />
                    </div>
                    <div>
                      ...
                      <div className="flex items-center flex-wrap gap-2 mt-3">
                        <span className="px-2 py-0.5 bg-surface-tertiary text-th-text-secondary text-xs rounded-full">
                          {doc.category}
                        </span>
                        {doc.tags?.slice(0, 3).map((tag) => (
                          <span
                            key={tag}
                            className="px-2 py-0.5 bg-th-accent-50 text-th-accent-700 text-xs rounded-full"
                          >
                            {tag}
                          </span>
                        ))}
        ...
        <div className="space-y-6">
          ...
          <div className="bg-surface-primary rounded-xl border border-th-border p-4">
            <h3 className="font-semibold text-th-text-primary mb-3">Categories</h3>
            <div className="space-y-1">
              <button
                onClick={() => setSelectedCategory(null)}
                className={`w-full flex items-center justify-between px-3 py-2 rounded-lg text-sm transition-colors ${
                  !selectedCategory
                    ? 'bg-th-accent-50 text-th-accent-700'
                    : 'text-th-text-secondary hover:bg-surface-tertiary'
                }`}
              >
                <span>All Documents</span>
                <span className="text-th-text-tertiary">{documents.length}</span>
              </button>
              {categories.map((cat) => (
                <button
                  key={cat.id}
                ...
                >
                  <span>{cat.name}</span>
                  <span className="text-th-text-tertiary">{cat.document_count || 0}</span>
                </button>
              ))}
            </div>
          </div>
          ...
          {popularDocs.length > 0 && (
            <div className="bg-surface-primary rounded-xl border border-th-border p-4">
              <div className="flex items-center space-x-2 mb-3">
                ...
                <h3 className="font-semibold text-th-text-primary">Popular</h3>
              </div>
              <div className="space-y-2">
                {popularDocs.map((doc) => (
                  <button
                    key={doc.id}
                    onClick={() => navigate(`/sops/${doc.id}`)}
                    className="w-full text-left px-3 py-2 rounded-lg hover:bg-surface-tertiary transition-colors"
                  >
                    <p className="text-sm font-medium text-th-text-secondary line-clamp-1">
                      {doc.title}
                    </p>
