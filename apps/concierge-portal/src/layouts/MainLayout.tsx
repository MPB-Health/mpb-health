              ...
              link.children ? (
                <div key={link.name} className="relative flex-1 flex justify-center">
                  <button
                    ...
                    className="flex items-center gap-1.5 px-4 py-2 rounded text-sm font-medium text-brand-sage hover:text-white hover:bg-white/10 transition-colors whitespace-nowrap"
                  >
                    {link.name}
                    <ChevronDown className={`w-3.5 h-3.5 transition-transform ${openDropdown === link.name ? 'rotate-180' : ''}`} />
                  </button>
                  {openDropdown === link.name && (
                    <div className="absolute top-full mt-1 left-1/2 -translate-x-1/2 bg-white rounded-lg shadow-xl border border-brand-sage/30 py-1 min-w-[200px] z-[9999]">
                      {link.children.map((child) => (
                        <a
                          ...
                          className="flex items-center justify-between gap-2 px-4 py-2 text-sm text-brand-forest hover:bg-brand-sage/15 transition-colors"
                        >
                          {child.name}
                          <ExternalLink className="w-3 h-3 text-brand-olive" />
                        </a>
                      ))}
                    </div>
                  )}
                ...
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex-1 flex items-center justify-center gap-1.5 px-4 py-2 rounded text-sm font-medium text-brand-sage hover:text-white hover:bg-white/10 transition-colors whitespace-nowrap"
                >
                  {link.name}
