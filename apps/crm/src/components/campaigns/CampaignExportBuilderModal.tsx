...
const FIELDS = [
  { id: 'name', label: 'Campaign Name', default: true },
  { id: 'type', label: 'Campaign Type', default: true },
  { id: 'status', label: 'Status', default: true },
  { id: 'budget', label: 'Budget', default: true },
  { id: 'actual_cost', label: 'Actual Cost', default: true },
  { id: 'roi', label: 'ROI', default: false },
  { id: 'members', label: 'Members Count', default: true },
  { id: 'leads', label: 'Leads Generated', default: false },
  ...
  { id: 'cpl', label: 'Cost per Lead', default: false },
];

const FORMATS = [
  { id: 'csv', label: 'CSV', icon: FileSpreadsheet, desc: 'Spreadsheet-ready' },
  { id: 'pdf', label: 'PDF', icon: FileText, desc: 'Print-ready report' },
  { id: 'print', label: 'Print', icon: Printer, desc: 'Direct to printer' },
];

export function CampaignExportBuilderModal({ open, onClose }: Props) {
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <div className="flex items-center gap-1.5 flex-wrap">
            {['all', 'active', 'paused', 'draft', 'scheduled', 'completed', 'cancelled'].map((s) => (
        ...
        <div className="p-2.5 rounded-xl bg-surface-secondary/50 border border-th-border/30 flex items-center gap-3">
          ...
          <div className="text-[10px] text-th-text-secondary"><strong>{selected.size} fields</strong> • <strong>{statusFilter === 'all' ? 'All' : statusFilter}</strong> campaigns • <strong>{format.toUpperCase()}</strong></div>
        </div>
