...
const FIELDS = [
  { id: 'name', label: 'Event Name', default: true },
  { id: 'type', label: 'Event Type', default: true },
  { id: 'date', label: 'Date', default: true },
  { id: 'location', label: 'Location', default: true },
  { id: 'contacts', label: 'Contacts Captured', default: true },
  { id: 'leads', label: 'Leads Generated', default: true },
  { id: 'rep', label: 'Rep Assigned', default: true },
  { id: 'cost', label: 'Event Cost', default: false },
  { id: 'roi', label: 'ROI', default: false },
  { id: 'conv_rate', label: 'Conversion Rate', default: false },
  { id: 'pipeline_status', label: 'Lead Pipeline Status', default: false },
  { id: 'follow_up', label: 'Follow-Up Status', default: false },
...
export function EventExportBuilderModal({ open, onClose }: Props) {
  ...
  return (
    <Modal open={open} onClose={onClose} title="Export Events" size="lg">
      <div className="space-y-4">
        ...
        <div>
          <label className="text-xs font-medium text-th-text-secondary block mb-1.5">Event Type</label>
          <div className="flex items-center gap-1.5 flex-wrap">
            {['all', 'church', 'hydration', 'chamber', 'health_fair', 'co_sponsored'].map((s) => (
              <button key={s} onClick={() => setTypeFilter(s)} className={cn('px-2.5 py-1 rounded-lg text-[10px] font-medium capitalize transition-all', typeFilter === s ? 'bg-th-accent-500/10 text-th-accent-500 border border-th-accent-500/30' : 'text-th-text-tertiary')}>{s === 'all' ? 'All' : s.replace('_', ' ')}</button>
            ))}
          </div>
        </div>
        <div>
        ...
        <div className="p-2.5 rounded-xl bg-surface-secondary/50 border border-th-border/30 flex items-center gap-3">
          ...
          <div className="text-[10px] text-th-text-secondary"><strong>{selected.size} fields</strong> • <strong>{typeFilter === 'all' ? 'All types' : typeFilter}</strong> • <strong>{format.toUpperCase()}</strong></div>
        </div>
