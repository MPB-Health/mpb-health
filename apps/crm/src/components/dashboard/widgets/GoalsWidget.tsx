...
export default function GoalsWidget({ config, size }: BaseWidgetProps) {
  ...
  if (goals.length === 0) {
    return (
      <div className="p-4 text-center text-gray-500">
        ...
        <button className="flex items-center gap-1 mx-auto mt-3 px-3 py-1.5 text-sm text-violet-600 hover:bg-violet-50 dark:hover:bg-violet-900/20 rounded-lg transition-colors">
  ...
  return (
    <div className="p-4">
          ...
          <div className="h-2 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden">
            <div
              className={cn(
                ...
                totalProgress >= 1 ? 'bg-green-500' : totalProgress >= 0.5 ? 'bg-violet-500' : 'bg-amber-500'
              )}
      ...
      <button className="flex items-center justify-center gap-1 w-full mt-4 pt-4 border-t dark:border-gray-700 text-sm text-violet-600 hover:text-violet-700 transition-colors">
        <Plus className="h-4 w-4" />
        Add new goal
      </button>
    </div>
...
const METRIC_COLORS: Record<string, string> = {
  ...
  deals_won: 'bg-purple-100 text-purple-600 dark:bg-purple-900/30',
  deals_value: 'bg-violet-100 text-violet-600 dark:bg-violet-900/30',
  ...
  tasks_completed: 'bg-indigo-100 text-indigo-600 dark:bg-indigo-900/30',
      ...
      <div className="flex items-center gap-3">
        <div className="flex-1 h-2 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden">
          <div
            className={cn(
              'h-full rounded-full transition-all',
              isComplete ? 'bg-green-500' : isOnTrack ? 'bg-violet-500' : 'bg-amber-500'
            )}
