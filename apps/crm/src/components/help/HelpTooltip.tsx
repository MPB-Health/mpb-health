...
export function HelpTooltip({
  ...
  const updatePosition = useCallback(() => {
    ...
    const shouldFlip = preferredSide === 'bottom' || (!preferredSide && rect.top < 120);
    setCoords({
    ...
    <>
      ...
      >
        <HelpCircle
          className={cn(
            iconSize,
            'text-th-text-tertiary/50 hover:text-th-accent-600 dark:hover:text-th-accent-400 transition-colors',
            iconClassName,
          )}
        />
      </span>
      {visible &&
        ...
        createPortal(
          ...
          >
            <div className="max-w-xs px-3.5 py-2.5 text-[11px] leading-relaxed text-white bg-neutral-800 dark:bg-neutral-100 dark:text-neutral-800 rounded-lg shadow-xl">
              {text}
              <div
                className={cn(
