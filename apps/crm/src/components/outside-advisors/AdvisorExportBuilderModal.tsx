...
const FIELDS = [
  ...
  { id: 'channel', label: 'Channel Type', default: true },
  { id: 'territory', label: 'Territory', default: false },
  { id: 'sourced', label: 'Leads Sourced', default: false },
  { id: 'closed', label: 'Cases Closed', default: false },
  { id: 'production', label: 'Production Volume', default: false },
  { id: 'override_rate', label: 'Override Rate', default: false },
  { id: 'compensation', label: 'Total Compensation', default: false },
  { id: 'performance', label: 'Performance Score', default: false },
  { id: 'last_activity', label: 'Last Activity', default: false },
  { id: 'onboarding', label: 'Onboarding Status', default: false },
...
export function AdvisorExportBuilderModal({ open, onClose }: Props) {
  ...
  return (
    <Modal open={open} onClose={onClose} title="Export Advisors" size="lg">
      <div className="space-y-4">
        ...
        <div className="p-2.5 rounded-xl bg-surface-secondary/50 border border-th-border/30 flex items-center gap-3">
          ...
          <div className="text-[10px] text-th-text-secondary"><strong>{selected.size} fields</strong> • <strong>{statusFilter === 'all' ? 'All' : statusFilter}</strong> advisors • <strong>{format.toUpperCase()}</strong></div>
        </div>
