...
const FIELDS = [
  { id: 'title', label: 'Task Title', default: true },
  { id: 'description', label: 'Description', default: true },
  { id: 'lead', label: 'Lead / Contact', default: true },
  { id: 'assigned', label: 'Assigned Rep', default: true },
  { id: 'due_date', label: 'Due Date', default: true },
  { id: 'status', label: 'Status', default: true },
  { id: 'priority', label: 'Priority', default: false },
  { id: 'type', label: 'Task Type', default: false },
  { id: 'source', label: 'Source (Auto/Manual)', default: false },
  { id: 'completed_date', label: 'Completed Date', default: false },
  { id: 'time_to_complete', label: 'Time to Complete', default: false },
  { id: 'overdue_days', label: 'Overdue Days', default: false },
...
export function TaskExportBuilderModal({ open, onClose }: Props) {
  ...
  return (
    <Modal open={open} onClose={onClose} title="Export Tasks" size="lg">
      <div className="space-y-4">
        ...
        <div className="p-2.5 rounded-xl bg-surface-secondary/50 border border-th-border/30 flex items-center gap-3">
          ...
          <div className="text-[10px] text-th-text-secondary"><strong>{selected.size} fields</strong> • <strong>{statusFilter === 'all' ? 'All' : statusFilter}</strong> tasks • <strong>{format.toUpperCase()}</strong></div>
        </div>
