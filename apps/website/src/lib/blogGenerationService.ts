...
export async function createBlogPost(
  ...
  try {
    ...
    const { data, error } = await supabase
      .from('blog_articles')
      .insert(insertData)
      .select('id, title, slug, excerpt, content, featured_image_url, category, author, is_published, published_date, read_time, scheduled_publish_at, created_at, updated_at')
      .single();
