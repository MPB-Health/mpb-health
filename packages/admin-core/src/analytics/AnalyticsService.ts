...
export class AnalyticsService {
  ...
  async exportReport(
    ...
    const rows_data = (data as unknown as Record<string, unknown>[]) || [];
    ...
    console.warn(
      `[PHI-EXPORT] type=${type} table=${table} records=${rows_data.length} range=${fromDate}..${toDate} user=${session?.user?.id ?? 'unknown'}`
    );
