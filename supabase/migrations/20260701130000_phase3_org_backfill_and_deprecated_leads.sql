-- Phase 3: Backfill stranded NULL org_id rows + deny-all on deprecated leads table
-- lead_submissions.org_id → orgs (membership UUID)
-- advisor_profiles.org_id → organizations (portal UUID)
...
BEGIN
  ...
  IF v_membership IS NULL OR v_portal IS NULL THEN
    RAISE EXCEPTION 'resolve_org_id returned NULL (membership=%, portal=%)', v_membership, v_portal;
  END IF;
